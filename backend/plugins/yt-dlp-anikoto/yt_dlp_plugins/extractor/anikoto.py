    ...
    def _entries(self, ani_id, title):
        ...
        for ep_info in self._get_all_episode_info(ani_id):
            ...
            for stream in self._get_stream_url(new_ep_info):
                # Each episode lists several mirror servers (vidtube/megaplay/
                # vidwish). Any one can be down or change format; skip the bad
                # ones rather than letting a single mirror kill the episode.
                try:
                    res = stream_ie.extract(stream['url'])
                except Exception as e:
                    self.report_warning(f'skipping server {stream["url"]}: {e}')
                    continue
...
class _MegaplayIE(InfoExtractor):
    ...
    _VALID_URL = (
        r'https?://(?:vidwish|megaplay|vidtube)\.(?:buzz|live|site)/stream/(?:s-\d+/)?(?P<id>[^/]+)/(?:h?sub|dub)(?:\?autostart=true)?'
    )
    ...
    def _real_extract(self, url: str):
        ...
        # base_url is the player origin (e.g. https://vidtube.site); used as the
        # referer and as the getSources host. Derive it from scheme+host so it's
        # robust to the path shape (older hosts had an extra /s-2/ segment).
